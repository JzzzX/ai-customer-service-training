...
本文件只适用于 `codex/learner-lite-sqlite` 分支，不能替代当前 `main` 的 Vercel + Neon 生产基线。
...
真实切换前先在维护窗口使用 `db:export:neon` 只读导出基线，再在空 SQLite 目标执行 `db:import:base` 和 `db:verify`。流水数据不在此次迁移范围内。不要改动 `main`、Vercel 或 Neon；若需要回到完整应用，用标签 `archive/full-app-before-learner-lite-20260810` 建立恢复分支。
