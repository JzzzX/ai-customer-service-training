...
阶段 1、阶段 2、阶段 3、阶段 4 和阶段 5 的工程实现与自动化验收均已完成，整体迁移进度按阶段计为 **100%**。阶段 5 的真实 MySQL 生产迁移、DNS 切换和旧系统下线必须在公司维护窗口、连接信息和审批具备后执行；在此之前旧 Next.js 系统保持生产权威。
...
| 5. 管理端与生产切换 | 100% | 已完成（工程验收） | 管理端、迁移对账、Nginx/systemd、双远程更新门禁和维护窗口 dry-run 已通过；真实生产窗口待外部条件 |
