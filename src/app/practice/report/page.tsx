...
export default async function PracticeReportPage({
  ...
  searchParams?: Promise<{ preset?: string; start?: string; end?: string; learnerId?: string; remediation?: string; category?: string; available?: string }>;
} = {}) {
  ...
  return (
    <main className="min-h-screen px-5 py-6 sm:px-8 sm:py-8">
      <div className="mx-auto max-w-5xl">
        ...
        {params.remediation === "insufficient-bank" ? <SoftCard className="mt-6 text-sm text-danger">{params.category} 当前可用题库不足 10 题（可用 {params.available ?? 0} 题），未生成考卷。</SoftCard> : null}
