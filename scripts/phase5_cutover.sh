...
if data.get("source", {}).get("hash") != data.get("target", {}).get("hash"):
    raise SystemExit("迁移 hash 不一致，禁止切换")
PY

if test "$dry_run" = true; then
  cat <<'EOF'
Phase5 maintenance-window dry-run
1. 将旧系统切换为只读并保留回滚入口
2. 导出旧系统最终增量并导入新系统
...
test "${PHASE5_CONFIRM_CUTOVER:-}" = "I_UNDERSTAND" || {
  echo "真实维护窗口必须显式设置 PHASE5_CONFIRM_CUTOVER=I_UNDERSTAND；脚本不会自动切 DNS 或删除旧系统" >&2
  exit 1
}
