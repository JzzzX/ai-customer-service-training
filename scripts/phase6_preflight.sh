...
if data.get("reconciled") is not True:
    raise SystemExit("迁移报告未通过 reconciled=true")
if target.get("foreign_key_orphans"):
    raise SystemExit("迁移报告包含外键孤儿记录")
if data.get("reconciliation", {}).get("checks") and not all(
    data["reconciliation"]["checks"].values()
):
    raise SystemExit("迁移报告 reconciliation checks 未全部通过")
PY
...
if test "${PHASE6_ALLOW_DIRTY:-false}" != "true"; then
...
echo "Phase6 production preflight passed"
