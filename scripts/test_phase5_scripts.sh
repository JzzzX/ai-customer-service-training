...
for script in phase5_preflight.sh phase5_cutover.sh; do
...
contains "$project_dir/scripts/phase5_cutover.sh" "I_UNDERSTAND"
...
printf '%s\n' '{"match":true,"source":{"hash":"same"},"target":{"hash":"same"}}' >"$manifest"
...
if APP_ENV=production DATABASE_URL=sqlite+pysqlite:///unsafe.db JWT_SECRET=short \
  ...
  PHASE5_ALLOW_DIRTY=true "$project_dir/scripts/phase5_preflight.sh" --manifest "$manifest" >/dev/null 2>&1; then
...
APP_ENV=production DATABASE_URL='mysql+pymysql://user:pass@localhost/db?charset=utf8mb4' \
  ...
  FEISHU_APP_CLIENT_ID=demo FEISHU_APP_CLIENT_SECRET=demo \
  PHASE5_ALLOW_DIRTY=true "$project_dir/scripts/phase5_preflight.sh" --manifest "$manifest" >/dev/null

output=$(PHASE5_ALLOW_DIRTY=true "$project_dir/scripts/phase5_cutover.sh" --dry-run --manifest "$manifest")
contains <(printf '%s' "$output") "旧系统切换为只读"
contains <(printf '%s' "$output") "最终增量"

if PHASE5_ALLOW_DIRTY=true "$project_dir/scripts/phase5_cutover.sh" --manifest "$manifest" >/dev/null 2>&1; then
  fail "非 dry-run 必须要求人工确认"
fi

echo "phase5 deployment scripts passed"
